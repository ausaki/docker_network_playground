...
create_interface () {
    ...
    sudo ip link add $interface type veth peer name "$interface-p" &>/dev/null || return
    give_interface_to_container "$interface-p" $container $short_name
...
create_point_to_point () {
    ...
    sudo ip link add "$1-$2" type veth peer name "$3-$4"
    give_interface_to_container "$1-$2" $1 $2
    give_interface_to_container "$3-$4" $3 $4
